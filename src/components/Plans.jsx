...
function PlanCard({ plan, index, inView }) {
  const { name, speed, price, description, features, cta, popular, badge } = plan
  ...
  return (
    ...
    >
      {/* Popular badge */}
      {badge && (
        <div className={`absolute top-0 right-0 px-4 py-1.5 text-xs font-bold rounded-bl-xl tracking-wide ${
          popular ? 'bg-brand-500 text-white' : 'bg-white/10 text-slate-300'
        }`}>
          {badge === 'Most Popular' && <Star size={10} className="inline mr-1 fill-current" />}
          {badge}
        </div>
      )}
      ...
      <div className="p-8 flex flex-col flex-1">
        {/* Plan header */}
        <div className="mb-6">
          <div className={`text-sm font-semibold uppercase tracking-widest mb-2 ${popular ? 'text-brand-400' : 'text-slate-500'}`}>
            {name}
          </div>
          <div className="flex items-end gap-1 mb-2">
            <span className="text-slate-400 text-lg font-medium">₱</span>
            <span className="font-display font-bold text-5xl text-white tabular-nums">{price}</span>
            <span className="text-slate-500 text-sm mb-2">/mo</span>
          </div>
          <div className={`flex items-center gap-2 font-semibold text-lg ${popular ? 'text-brand-300' : 'text-slate-200'}`}>
            <Zap size={18} className={popular ? 'text-brand-400' : 'text-slate-400'} />
            {speed}
          </div>
        ...
        {/* Divider */}
        <div className={`border-t mb-6 ${popular ? 'border-brand-500/30' : 'border-white/[0.06]'}`} />
          ...
          {features.map(({ text, included }) => (
            <li key={text} className="flex items-start gap-3">
              <span className={`shrink-0 mt-0.5 w-5 h-5 rounded-full flex items-center justify-center ${
                included
                  ? popular ? 'bg-brand-500/20 text-brand-400' : 'bg-brand-900/40 text-brand-500'
                  : 'bg-white/5 text-slate-600'
              }`}>
                {included ? <Check size={12} /> : <X size={12} />}
              </span>
              <span className={`text-sm ${included ? 'text-slate-300' : 'text-slate-600 line-through'}`}>
                {text}
              </span>
            </li>
          ))}
    ...
    <section id="plans" className="py-24 relative">
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="text-center mb-12">
          ...
          >
            No hidden fees. No contracts required. Cancel anytime with 30 days notice.
          </motion.p>
        ...
        >
          All plans include free installation, professional setup, and a 30-day money-back guarantee.
          <br />
